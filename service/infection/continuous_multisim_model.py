...
class ContinuousMultisimModel(AbstractSimModel):
    ...
    def prepare_testing(self):
        for day, true_I in enumerate(self.daily_infected):
            ...
            test_positive_n = min(self.daily_tests_positive,
                                  positive_candidates.sum())
            test_random_n = self.daily_tests_random + \
                self.daily_tests_positive - test_positive_n
    ...
    def apply_testing(self):
        ...
        self.R_t[new_I] = np.random.geometric(
            1 / self.recovery_t, new_I.sum()) + self.today
    ...
    def run_sim(self):
        with self.tqdm(total=self.n_days) as pbar:
            ...
            for day, edges in self.edge_batch_gen:
                ...
                passthrough = self.random_binary_array(
                    spread_I.shape, self.infection_p)
                ...
                for node, node_I in new_I.items():
                    ...
                    self.R_t[node, node_I] = \
                        np.random.geometric(
                            1/self.recovery_t, node_I.sum()) + self.today
                    # self.R_t[node, node_I] = \
                ...
                if not self.I.any():
                    ...
                    print(
                        f'No more infected nodes in any simulation, terminating on day {day}.')
                    break
    ...
    def plot_stats(self, stacked=True):
        ...
        if stacked:
            snaps.plot.area(stacked=True, alpha=0.4,
                            color=colors, ylim=(0, 100), ax=self.plt_ax)
        else:
